...
public class VoiceMsgHandler extends AbstractHandler {
    ...
    public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage,
        ...
        //语音消息识别结果
        String content = wxMessage.getRecognition();
        log.info("语音消息识别结果==="+content);
        content = content.substring(0,content.length()-1);
        log.info("切换文本==="+content);
        WxMpXmlOutMessage returnmse = handle(wxMessage,weixinService,content);
        if(returnmse.getMsgType().equals(WxConsts.XmlMsgType.TEXT)){
            WxMpXmlOutTextMessage msg = (WxMpXmlOutTextMessage) returnmse;
            String newcontent = "语音消息识别结果:"+content +"\n"+ msg.getContent();
...
//        }
    ...
    public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage, WxMpService weixinService,String content) {
        ...
        if(keywordList!=null && keywordList.size()>0){
            MpwxReply mpwxReply = (MpwxReply) mpwxReplyService.getById(keywordList.get(0).getReplyId());
            return replyUtil.mpwxReplyToWxMpXmlOutMessage(mpwxReply,wxMessage, weixinService,null);
        }else {
            //回复默认消息
            //测试使用
//             return new TextBuilder().build("<a href=\"weixin://bizmsgmenu?msgmenucontent=密码类&msgmenuid=123\">密码类</a>", wxMessage, weixinService);

            return replyUtil.getDefautMessage(wxMessage, weixinService,null);
        }
